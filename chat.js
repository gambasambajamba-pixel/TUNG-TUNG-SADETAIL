(() => {
  ...
  if (services) services.insertAdjacentHTML('afterend', `<section class="tt-season-feature"><div class="tt-season-feature-inner"><div><div class="eyebrow">${campaign.icon} · ${campaign.title}</div><h2>${campaign.season === 'summer' ? 'Beach days are better with a fresh ride.' : 'A seasonal refresh for your ride.'}</h2><p>${campaign.season === 'summer' ? 'From sandy floors to road-trip crumbs and summer dust, book the detail that makes every drive feel better.' : campaign.copy}</p><a href="#booking">${campaign.cta} ↗</a></div><div class="tt-season-points"><article class="tt-season-point"><b>${campaign.season === 'summer' ? '01' : '01'}</b><strong>${campaign.season === 'summer' ? 'Beach-day cleanup' : 'Seasonal refresh'}</strong></article><article class="tt-season-point"><b>${campaign.season === 'summer' ? '02' : '02'}</b><strong>${campaign.season === 'summer' ? 'Road-trip ready' : 'Ready for every drive'}</strong></article><article class="tt-season-point"><b>${campaign.season === 'summer' ? '03' : '03'}</b><strong>${campaign.season === 'summer' ? 'Cool, clean interior' : 'Clean inside and out'}</strong></article></div></div></section>`);
