(() => {
  ...
  if (services) services.insertAdjacentHTML('beforebegin', '<section class="tt-spin" id="spin"><div class="tt-spin-inner"><div><div class="eyebrow" style="color:#8fc2ff">Tung Tung rewards</div><h2>Spin for a future detail reward.</h2><p>Sign up once with your email or number. The reward spaces are being finalized and will be announced soon.</p><form class="tt-spin-form" id="tt-spin-form"><input id="tt-spin-contact" required autocomplete="email" placeholder="Email or mobile number"><button type="submit">Unlock one spin</button></form><p class="tt-spin-note">Your entry opens as a text message for you to send. One spin per browser.</p></div><div class="tt-wheel" id="tt-wheel" aria-label="Reward wheel"><span>SPIN<br>SOON</span></div></div></section>');
  ...
  if (spinForm) spinForm.addEventListener('submit', event => {
    ...
    setTimeout(() => { spinForm.innerHTML = '<strong style="color:#fff;font-size:14px">You unlocked your one spin! Send the text to finish signing up. Rewards will be announced soon.</strong>'; }, 900);
  });
